...
declare interface X32 {
  ...
  on(event: 'populated', listener: () => void): this;
  on(event: 'osc', listener: (oscObj?: oscObject) => void): this;
}
...
class X32 extends EventEmitter {
  ...
  constructor(X32_address: string, localAddress?: string, localPort?: number) {
    super();
    this.on('connected', () => {
      this.connected = true;
    });
  ...
  getNode(index: number, address?: string) {
    return new Promise(
      (res: (msgs: oscMessage[]) => void, rej: (err: string) => void) => {
        let command = X32.strToBuf('/node');
        let format = X32.strToBuf(',s');
        if (!address) address = x32nodes[index];
        let splitter = address!.indexOf('[');
        if (splitter == -1) splitter = address!.indexOf('{');
        if (splitter == -1) {
          ...
          let retry = setInterval(() => {
            ...
            if (attempts > 5) {
              ...
              rej(
                'X32 has not responded after 5 attempts. Failed message:' +
                  message.toString()
              );
              clearInterval(retry);
            }
          ...
          let attempts = 1;
          let checker: checker = (msg: oscMessage, x) => {
          ...
          if (infix.length == 2 || splitString == ',') {
            if (splitString == '...') {
              ...
              for (let i = range[0]; i <= range[1]; i++) {
                ...
                while (infixElem.length < range[2]) infixElem = '0' + infixElem;
  ...
  static strToBuf(str: string): Buffer {
    let buf = Buffer.from(str);
    let bufPad = Buffer.alloc(4 - (buf.length % 4));
    return Buffer.concat([buf, bufPad]);
  }

  static bufToObj(buf: Buffer): oscMessage | string {
    ...
    switch (command) {
      ...
      default:
        ...
        command = 'xremote';
        if (xaddress[0] == '-') return 'unneeded update';
        let xformat = X32.getFormat(xaddress);
        if (xformat.slice(0, 5) == 'error') return xformat;
        index = buff.indexOf(0x00);
        ...
        if (xformat[0] == '[') {
          ...
          if (formatArr[0] == 'int' && typeof formatArr[1] == 'number') {
            //int
            if (oscFormat != 'i') return 'error: invalid osc format';
            ...
            while (xvalue.length < formatArr[1]) xvalue = '0' + xvalue;
            xvalue = '%' + xvalue;
          } else if (formatArr.length == 3 && typeof formatArr[0] == 'number') {
            //linf
            if (oscFormat != 'f') return 'error: invalid osc format';
  ...
  static getFormat(address: string, obj?: any): string {
    if (!obj) obj = x32json;
    ...
    if (index == -1) {
      ...
      if (newRoot.length <= 3) newRoot = X32.digitsToSlash(newRoot);
      if (obj[newRoot]) {
  ...
  static getLeafIndex(key: string, arr: any[]): number {
    let index = -1;
  ...
  static parseNode(str: string): { path: string[]; values: string[] } {
    ...
    if (tempValues.length > 0) {
      ...
      for (let x = 0; x < tempValues.length; x++) {
        ...
        if (values[i].split('"').length - 1 != 1 && values[i] != '') {
          ...
          i++;
          if (x < tempValues.length - 1) values[i] = '';
          //console.log(tempValues.length + ' ' + x)
        }
      }
    } else console.error('OSC path with no arguments');
    return { path: path, values: values };
  }

  static oscToObject(osc: string[], returnObj?: oscObject) {
  ...
  static addToObject(
    ...
    if (path.length == 0 || val.length == 0) return;
    if (path.length == 1) {
  ...
  static getKeyArray(
    pathIn: string[],
    x32Obj?: any
  ): oscKeyArray | 'atLeaf' | 'missing path' {
    let path = pathIn.slice(0); //don't mutate the argument
    if (!x32Obj) x32Obj = x32json;
    let newRoot = path.shift()!;
      ...
      if (x32Obj[newRoot] && x32Obj[newRoot].leaves) {
        ...
        if (x32Obj.leaves) {
          for (let x = 0; x < x32Obj.leaves.length; x++) {
            if (x32Obj.leaves[x].name == oldRoot) return 'atLeaf';
          }
        }
        console.log(newRoot);
        return 'missing path';
  ...
  static digitsToSlash(str: string): string {
    let convert = true;
    let rtn = str;
    let strArray = Array.from(str);
    for (let x = 0; x < strArray.length; x++) {
      if (parseInt(strArray[x]).toString() != strArray[x]) convert = false;
    }
    if (convert) {
      for (let x = 0; x < strArray.length; x++) {
        strArray[x] = '/';
  ...
  static getDiffObj(ref: oscObject, osc: oscObject): diffObject {
  ...
  static getTinyDiffObj(ref: oscObject, osc: oscObject): diffObject {
    let returnObj: diffObject = {};
    for (const key in ref) {
